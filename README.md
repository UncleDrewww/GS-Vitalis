...
服务连接成功后，您可以在 Cherry Studio 的对话框中输入自然语言指令，系统将通过 MCP 工具自动调用后台数据进行分析。

### 📝 示例：卫星健康状态评估
请在聊天框中输入以下指令：

> **"帮我评估江淮2025年11月的卫星健康状态"**

**预期运行逻辑：**
1.  **意图识别**：模型识别到需要查询特定时间（2025年11月）和特定卫星（江淮）的状态。
2.  **工具调用**：自动调用在轨数据挖掘系统的 API 接口。
3.  **结果反馈**：系统将检索遥测数据，分析该时间段的健康指标，并返回评估报告。
